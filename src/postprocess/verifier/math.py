...
def last_boxed_only_string(string: str) -> str | None:
    ...
    return (
        string[idx : right_brace_idx + 1] if right_brace_idx is not None else ""
    )  # None
...
def verify(
    solution_str: str, answer: str, pause_tokens_index: list[int] | None = None
) -> tuple[bool, str]:
...
def compute_score(
    ...
    if incorrect_format and not was_truncated and format_feedback:
        feedback = r"Your answer had the wrong format. The solution must be given in the format: \boxed{your_answer}."
