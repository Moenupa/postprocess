...
from postprocess.verifier.code.code_utils import extract_code, reliability_guard
...
def set_memory_limits(maximum_memory_bytes: int | None) -> None:
    """
    ...
    try:
        ...
        if hasattr(_resource, "RLIMIT_AS"):
            _resource.setrlimit(
                _resource.RLIMIT_AS, (maximum_memory_bytes, maximum_memory_bytes)
            )
        ...
        for limit_name in ("RLIMIT_DATA", "RLIMIT_RSS"):
            if hasattr(_resource, limit_name):
                ...
                try:
                    _resource.setrlimit(
                        limit_const, (maximum_memory_bytes, maximum_memory_bytes)
                    )
                except Exception:
...
def _build_restricted_builtins():
    ...
    allowed_names["open"] = None  # deny file I/O  # ty:ignore[invalid-assignment]
...
def run_tests_for_one_example(
    test_cases, completion, send_conn, sparse_rewards, test_idx
):
    ...
    try:
        ...
        try:
            ...
            if test_type == "functional":
                ...
                passed, output_value = run_test_code(
                    completion, copy.deepcopy(test_input), globals
                )
            else:
...
def format_test_feedback(
    ...
    selected = (
        _first(
            lambda rec: (
                isinstance(rec.get("actual"), str)
                and str(rec.get("actual")).startswith(ERROR_PREFIX)
            )
        )
    ...
    if selected is not None:
        ...
        if sort_test_cases_by_length:
            failing = sorted(
                failing, key=lambda x: len(str(x["input"])) + len(str(x["actual"]))
            )
...
def run_tests(test_cases: dict, solution, sparse_rewards, max_test_cases):
    ...
    num_test_cases = (
        min(max_test_cases, len(test_cases["inputs"]))
        if max_test_cases
        else len(test_cases["inputs"])
    )
    timeout_per_test_case = (
        float(test_cases["time_limit"])
        if test_cases["time_limit"] is not None
    ...
    for test_idx in range(num_test_cases):
        ...
        process_data.append((p, parent_conn))

    start_time = time.time()
    for test_idx, (p, parent_conn) in enumerate(process_data):
        ...
        timeout_this_test = max(
            0, timeout_per_test_case * TIMEOUT_SCALER + 1 - (time.time() - start_time)
        )
...
def compute_score(
    ...
    incorrect_format = (
        ...
        and (records[0]["actual"] == INCORRECT_FORMAT)
    )
    error_in_test_cases = any(
        [
            (
                (not r["passed"])
                and isinstance(r["actual"], str)
                and ERROR_PREFIX in r["actual"]
            )
            for r in records
        ]
    )
